...
public class Parqueadero {
    ...
    private int buscarPuestoLibre( )
    {
        ...
        for( int i = 1; i < TAMANO && puesto == NO_HAY_PUESTO; i++ )
        {
    ...
    public double darTiempoPromedio() {
        ...
        for (Puesto puesto : puestos) {
            if (puesto.estaOcupado()) {
                ...
                tiempoTotal += tiempoEnParqueadero;
                totalCarros++;
            }
        }
    ...
    public boolean hayCarroMasDeOchoHoras() {
        boolean hayCarroMasDeOchoHoras = false;
        ...
        for (Puesto puesto : puestos) {
            if (puesto.estaOcupado()) {
                ...
                if (tiempoEnParqueadero > 8) {
                    hayCarroMasDeOchoHoras = true;  // Hay un carro que lleva más de 8 horas parqueado
                    break;  // Salir del bucle al encontrar un carro
                }
            }
        }

        if (hayCarroMasDeOchoHoras) {
            System.out.println("Sí, hay carros parqueados más de 8 horas.");
        } else {
            System.out.println("No hay ningún carro estacionado por más de 8 horas.");
        }

        return hayCarroMasDeOchoHoras;
    }
    ...
    public ArrayList<Carro> darCarrosMasDeTresHorasParqueados() {
        ...
        boolean hayCarroMasDeTresHoras = false;
        ...
        for (Puesto puesto : puestos) {
            if (puesto.estaOcupado()) {
                ...
                if (tiempoEnParqueadero > 3) {
                    carrosMasDeTresHoras.add(carro);  // Agregar carro a la lista
                    hayCarroMasDeTresHoras = true;   // Marcar que se encontró al menos un carro
                }
            }
        }
        if (!hayCarroMasDeTresHoras) {
            System.out.println("No hay ningún carro estacionado por más de tres horas.");
        }
    ...
    public boolean hayCarrosPlacaIgual() {
        boolean hayCarrosPlacaIgual = false;

        int totalCarros = 0;

        for (int i = 0; i < puestos.size(); i++) {
            if (puestos.get(i).estaOcupado()) {
                Carro carroI = puestos.get(i).darCarro();

                for (int j = i + 1; j < puestos.size(); j++) {
                    if (puestos.get(j).estaOcupado()) {
                        Carro carroJ = puestos.get(j).darCarro();

                        if (carroI.darPlaca().equalsIgnoreCase(carroJ.darPlaca())) {
                            hayCarrosPlacaIgual = true;
                            System.out.println("Hay dos carros con la misma placa");
                            break;
                        }
                    }
                }
        ...
        if (!hayCarrosPlacaIgual) {
            System.out.println("No hay dos carros con la misma placa.");
        }

        return hayCarrosPlacaIgual;
    }

    public boolean hayCarroCon24Horas() {
        boolean hayCarroCon24Horas = false;
        ...
        for (Puesto puesto : puestos) {
            if (puesto.estaOcupado()) {
                ...
                if (tiempoEnParqueadero >= 24) {
                    hayCarroCon24Horas = true;  // Hay un carro que lleva 24 o más horas parqueado
                    break;  // Salir del bucle al encontrar un carro
                }
            }
        }

        return hayCarroCon24Horas;
    }


    public int desocuparParqueadero() {
        int cantidadCarrosSacados = 1;
    ...
    public String metodo1() {
        ...
        if (hayCarroCon24Horas) {
            return "Cantidad de carros con placa PB: " + cantidadCarrosPB + " – Hay carro parqueado por 24 o más horas: Sí.";
        } else {
            return "Cantidad de carros con placa PB: " + cantidadCarrosPB + " – Hay carro parqueado por 24 o más horas: No.";
        }
    }
